...
getQueerVectors()
  .then(async fetchedQueerData => {
    ...
    const batchSize = 100; // Define a batch size to process/render at a time
    let startIndex = 0;
  

    // 
    //while (startIndex < fetchedQueerData.length) {
     // const endIndex = Math.min(startIndex + batchSize, fetchedQueerData.length);
     // const batchData = fetchedQueerData.slice(startIndex, endIndex);
  
 //     await processAndRenderBatch(batchData); // Your function to process and render a batch
...
function interpolatedData(data2){
  ...
  data2.forEach(vector2 => {
    ...
    const material2 = new THREE.MeshBasicMaterial({ color: '#ff00fc' });
